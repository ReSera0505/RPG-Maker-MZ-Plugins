...
(() => {
    ...
    window.RSTH_IH.findInsertSlot = function (slots, item, count = 1) {
        ...
        for (let i = 0; i < slots.length; i++) {
            const slot = slots[i];
            if (slot && slot.id === item.id && slot.type === type && slot.count + count <= window.RSTH_IH.StackSize) {
                return i;
            }
        }
    ...
    window.RSTH_IH.checkPlaceBlockAt = function (x, y) {
        // マップ範囲内チェック
        ...
        // 既にタイルが埋まっていないか（仮: tileIdが0以外ならブロックありとする）
        const currentTileId = $gameMap.tileId(x, y, 0); // レイヤー0
        if (currentTileId !== 0) return false;
