        ...
        try {
            ...
            for (const offset of tileOffsets) {
                ...
                if (existingBlock) {
                    ...
                    const allowed = (
                        (baseType === "ground" && ["floor", "wall", "furniture", "plant", "chest", "workbench"].includes(placingType)) ||
                        (baseType === "floor" && ["wall", "furniture", "plant", "chest", "workbench"].includes(placingType))
                    );
                    if (!allowed) {
