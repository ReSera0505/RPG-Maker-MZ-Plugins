    ...
    window.RSTH_IH.SurvivalBlockManager = {
        ...
        place(x, y, itemId) {
            ...
            if (item.meta.autoTile === "true") {
                ...
                const thistileType = Number(item.meta.tileType);
                ...
                for (const [dx, dy] of neighborOffsets) {
                    ...
                    const neighborTileType = neighbor ? Number(neighborItem.meta.tileType) : 0;
                    ...
                    if (neighbor && neighborItem?.meta?.autoTile === "true" &&
                        ...
                        if (RSTH_DEBUG_LOG) console.log(`[place] autoTileリフレッシュ予約 (${nx}, ${ny})`);
                    }
                }

                // 自分自身も必ずリフレッシュ
                this._spriteRefreshQueue.push({ x: cx, y: cy });
                if (RSTH_DEBUG_LOG) console.log(`[place] autoTileリフレッシュ予約（中心） (${cx}, ${cy})`);
            }
            ...
            if (autoTileBlock.meta.autoTile === "true") {
                ...
                for (const [dx, dy] of neighborOffsets) {
                    ...
                    if (neighbor && neighborItem?.meta?.autoTile === "true" &&
                        ...
                        neighborTileType === thistileType) {
                        this._spriteRefreshQueue.push({ x: nx, y: ny });
                        if (RSTH_DEBUG_LOG) console.log(`[break(x, y)]  autoTileリフレッシュ予約 (${nx}, ${ny})`);
                    }
                }

                // 自分自身も必ずリフレッシュ
                this._spriteRefreshQueue.push({ x: cx, y: cy });
                if (RSTH_DEBUG_LOG) console.log(`[break(x, y)]  autoTileリフレッシュ予約（中心） (${cx}, ${cy})`);
            }
        ...
        addSprite(block, force = false) {
            ...
            if (item.meta.autoTile === "true") {
                ...
                if (RSTH_DEBUG_LOG) console.log(`[addSprite] autoTile_sprite`, sprite);
            } else {
                sprite = new window.RSTH_IH.Sprite_SurvivalBlock(block);
                sprite.bitmap = ImageManager.loadTileset(tilesetName);
                if (RSTH_DEBUG_LOG) console.log(`[addSprite] normal_sprite`, sprite);
            }
            ...
            // dx, dy を block に渡しておく（Autotile側で使用）
            block.dx = dx;
            block.dy = dy;

            if (!(sprite instanceof window.RSTH_IH.Sprite_AutotileBlock)) {
            ...
            if (blockZ === "under") {
                ...
                switch (blockType) {
                    ...
                    case "chest": sprite.z = 3; break;
                    case "workbench": sprite.z = 3; break;
            ...
            if (RSTH_DEBUG_LOG) window.RSTH_IH.getBlocks_and_Spriteslist("addSprite 完了");
